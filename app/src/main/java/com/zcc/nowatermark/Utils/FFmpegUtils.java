...
//        [1:a]aformat=sample_fmts=fltp:sample_rates=44100:channel_layouts=stereo,volume=0.5[a1];[a0][a1]amix=inputs=2:duration=first[aout]" -map "[aout]" -ac 2 -c:v copy -map 0:v:0 output.mp4
     ...
     * @param h 去处区域高
     */
    public static void cleanWaterMark(String srcVideoPath, int x, int y, int w, int h, String outputPath, OnEditorListener onEditorListener) {
        CmdList commandList = new CmdList();
        ...
        long d = VideoUitls.getDuration(srcVideoPath);
        Log.e("commandList===",commandList.toString());
        execCmd(commandList, d, onEditorListener);
    }
